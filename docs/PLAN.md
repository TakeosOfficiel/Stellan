...
Tranche suivante réalisée pour la conversation en file : plusieurs messages peuvent être persistés pendant qu’un run du même thread travaille. SQLite conserve leur ordre et leur contenu ; le processus principal permet de modifier ou supprimer uniquement une entrée encore en attente, de la placer en tête avec **Envoyer maintenant**, puis d’interrompre le run courant avant de la démarrer. Le contexte de chaque run est reconstruit jusqu’à son propre message et exclut les demandes futures. L’interface expose une vue regroupant les états en attente, en cours, terminés, interrompus et en erreur.
