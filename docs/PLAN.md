...
État actuel de la reprise agent : le processus principal journalise dans SQLite chaque exécution et les transitions ordonnées des appels d’outils avec leurs arguments et résultats. Une annulation, une erreur ou un redémarrage marque atomiquement l’exécution et les outils encore actifs comme interrompus. Le contexte envoyé au modèle est reconstruit depuis cet historique principal puis borné déterministement à 18 000 caractères avec Ollama, ou adapté au contexte exposé par llama.cpp, en conservant les échanges récents et les paires appel/résultat ; les éléments surdimensionnés sont tronqués, sans prétendre produire un résumé sémantique. La reprise automatique d’une génération interrompue et la réduction sémantique des anciens échanges restent à réaliser.
...
llama.cpp est le moteur d’inférence principal pour les modèles dont Stellan référence un artefact GGUF vérifié. Il sélectionne CUDA, ROCm, Vulkan puis le CPU selon le matériel et revient automatiquement à Ollama si son démarrage échoue. Ollama reste utilisé pour le téléchargement, la vision non couverte, la génération d’images, les modèles sans artefact vérifié et le moteur de secours. Les appels d’outils, les retours d’outils et les passes successives du modèle utilisent la même couche d’inférence indépendante du moteur. vLLM et SGLang restent hors périmètre : leur avantage concerne surtout la concurrence serveur, pas l’agent local mono-utilisateur.
