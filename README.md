...
7. Pendant une génération, envoyer plusieurs messages, ouvrir **Historique et file**, modifier puis supprimer une entrée et utiliser **Envoyer maintenant**. Fermer puis rouvrir l’application : la génération active devient interrompue, tandis que les messages encore en attente sont conservés et reprennent dans l’ordre. Une confirmation supplémentaire protège les changements non enregistrés lors de la suppression du thread.
...
La file d’un thread appartient également au processus principal et à SQLite, pas au renderer. Chaque message est persisté avant confirmation, un seul message de ce thread peut être exécuté à la fois et les réponses futures ne sont jamais injectées dans le contexte du message courant. **Envoyer maintenant** place l’entrée choisie en tête, interrompt proprement la génération courante, puis la démarre ; les autres entrées gardent leur ordre. À la fermeture, seul le run réellement actif est marqué interrompu et les entrées en attente restent reprises au prochain lancement.
